...
#include "XGLoginClientLockOneACAA.h"
...
UXGLoginClientLockOneACAA::UXGLoginClientLockOneACAA(const FObjectInitializer& ObjectInitializer)
:Super(ObjectInitializer)
{

}

UXGLoginClientLockOneACAA::~UXGLoginClientLockOneACAA()
{
	RealeaseResources();
}

UXGLoginClientLockOneACAA* UXGLoginClientLockOneACAA::XGLoginClientLockOneACAA(UObject* WorldContextObject,
const FString& InManagerName,
const FString& InManagerPassword,
const FString& InActiveCode)
{
	UXGLoginClientLockOneACAA* AsyncAction = NewObject<UXGLoginClientLockOneACAA>();
...
void UXGLoginClientLockOneACAA::Activate()
{
	Super::Activate();

	Then.Broadcast(AsyncID, false,
		TEXT("XGLoginClientLockOneACAA is just started,please wait to be finished!"));

}

void UXGLoginClientLockOneACAA::Activate_Internal()
{

	EXGLoginServerResult ResponseResult;

	//1.校验数据正确与否
	FXGLockOneStatusReqData ReqData;
	...
	//4.发出请求
	SendHttp(EXGLoginRequestType::LockOneStatusReq, ReqDataStr);

}

void UXGLoginClientLockOneACAA::SendHttp(EXGLoginRequestType InRequestType, const FString& InRequestDataStr)
{
	...
	Request->OnProcessRequestComplete().BindUObject(this, &UXGLoginClientLockOneACAA::OnHttpRespReceived, LoginServerRequest.RequestType);
...
void UXGLoginClientLockOneACAA::OnHttpRespReceived(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, EXGLoginRequestType RequestType )
{
	UE_LOG(LogXGLoginClient, Display, TEXT("[%s],AsyncID:[%s],Receive LockOne HttpResonpse"), *FString(__FUNCTION__), *AsyncID.ToString());
...
void UXGLoginClientLockOneACAA::CallOnSuccess(FGuid InAsyncID, bool bInResult, FString InMessage)
{
	FXGLoginClientLockOneACDelegate TempDelegate = OnSuccess;
...
void UXGLoginClientLockOneACAA::CallOnFail(FGuid InAsyncID, bool bInResult, FString InMessage)
{
	FXGLoginClientLockOneACDelegate TempDelegate = OnFail;
...
void UXGLoginClientLockOneACAA::RealeaseResources()
{
